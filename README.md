...
- `牛牛在吗`: 仅超管可用,查看目前在线的牛牛,配置邮箱后还可接收掉线提醒 [bot_status](src/plugins/bot_status/README.md)

以下功能仅当部署了 [AI 服务端](https://github.com/PallasBot/Pallas-Bot-AI) 且开启对应功能时可用：

- `牛牛唱歌 <网易云歌曲 ID 或 歌名>`：AI 牛牛翻唱！（人声提取 + 音色转换）
- `牛牛点歌 <网易云歌曲 ID 或 歌名>`：要知道牛牛唱的怎么样?听听原曲吧!
- `网易云登录` & `网易云登出`：想要牛牛唱VIP歌曲,需要配置一个网易云VIP账号
